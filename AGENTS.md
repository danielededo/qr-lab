...
decoding, error correction) through standalone subprojects: `wifi-qr`
(WiFi credential QR codes), `totp-qr` (TOTP/2FA setup QR codes with a
from-scratch RFC 6238 implementation), and `qr-damage` (an error-correction
lab that damages QR codes until they stop decoding). Optimized for clarity and
hands-on understanding, not production use. Python 3.11+.
...
- Setup: `pip install -r wifi-qr/requirements.txt -r totp-qr/requirements.txt -r qr-damage/requirements.txt`
- Lint: `ruff check .`
- Smoke test (no test framework — round-trip the real scripts):
  ...
  - `python qr-damage/damage.py --levels LH --trials 2 --step 0.02` (both experiments must complete)
...
- `qr-damage/` — error-correction lab: damage QR codes until they stop decoding
